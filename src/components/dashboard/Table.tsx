		...
		<Container className='mb-3'>
			...
			<Grid
				data={property.map((prop, i) => [
					...
					_(
						<>
							<Button className='' size='sm' variant='info' onClick={() => onDetail(prop._id)}>
								<IoMdEye />
							</Button>
							<Button className='mx-1' size='sm' variant='warning' onClick={() => onUpdate(prop._id)}>
								<IoMdCreate />
							</Button>
							<Button className='' size='sm' variant='success' onClick={() => onModal(prop._id, prop.title, 'soldOut')}>
								<IoMdCheckmark />
							</Button>
							<Button
								className='ml-1'
								size='sm'
								variant='danger'
								onClick={() => onModal(prop._id, prop.title, 'delete')}
							>
								<IoMdTrash />
							</Button>
						</>
